...
* 14-nov-10 bwillett created alpha-q.c, include a z flattening q
*
 */
...
void inipotential (int *npar, double *par, string name)
{
    ...
    if (n>7) haloa = par[7];
    if (n>8) q = par[8];
    if (n>9) warning("mpl: only first 8 parameters recognized");

}
    
void potential_double(int *ndim,double *pos,double *acc,double *pot,double *time)
{
    double apar, qpar, spar, ppar, lpar, rpar, rcyl, rsq, rpar2;
    int i;
...
// 20070507 bwillett used hypot from math.h
    ...
    rpar = ppar - plu_rc; 
    rsq = rpar*rpar;
    rpar2 = sqrt ((pos[X]*pos[X])+(pos[Y]*pos[Y])+(pos[Z]*pos[Z])/(q*q));
	
// 20070312 bwillett added plummer sphere potential and logarithmic potential
    *pot = (-(miya_mass)/sqrt(spar)) + (-(plu_mass)/ppar) + (-haloa * pow(rpar2,2.0-alpha)/((2.0-alpha)*(alpha-3.0)));
//    *pot = 1;
...
// I did these derivatives in Mathematica, and will try to keep it consistent with the conventions written above
	...
	acc[X] = - (( (plu_mass*pos[X])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[X])/(pow(spar,1.5)) ) ) + ((haloa*pos[X]*pow(rpar2,-alpha))/(alpha-3.0));
	acc[Y] = - ( ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Y])/(pow(spar,1.5)) ) ) + ((haloa*pos[Y]*pow(rpar2,-alpha))/(alpha-3.0));
	acc[Z] = - ( ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Z]*apar) / (qpar*pow(spar,1.5)) ) ) + ((haloa*(pos[Z]/(q*q))*pow(rpar2,-alpha))/(alpha-3.0));
