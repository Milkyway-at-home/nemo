 ...
 * 27-aug-09 bwillett created expdisklog.c, changed disk to exponential disk
*
 */
...
{
    double apar, qpar, spar, ppar, lpar, rpar, rcyl, diskpar;
    int i;
...
diskpar = (disk_mass * exp(-rpar/disk_b) * (1.0 - exp(rpar/disk_b) + (rpar/disk_b))) / (rpar*rpar*rpar);

	acc[X] = - ( ( (2.0*vhalo*vhalo*pos[X])/(lpar) ) + ( (plu_mass*pos[X])/(rpar*ppar*ppar) ) - (diskpar*pos[X]) );
	acc[Y] = - ( ( (2.0*vhalo*vhalo*pos[Y])/(lpar) ) + ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) - (diskpar*pos[Y]) );
	acc[Z] = - ( ( (2.0*vhalo*vhalo*pos[Z])/(q*q*lpar) ) + ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) - (diskpar*pos[Z]) );
