...
*   11-dec-10 bwillett created lumpy.c, no log component, 15 dark matter lumps 
*
 */
...
// 20070312 bwillett added plummer sphere potential and logarithmic potential
    *pot = (-(miya_mass)/sqrt(spar)) + (-(plu_mass)/ppar);
//    *pot = 1;

// Add the lump contributions to the potential

	for(i=0;i<=14;i++) {

    		tmp = 1.0/(la[i]*la[i] + (pos[0]-lX[i])*(pos[0]-lX[i]) + (pos[1]-lY[i])*(pos[1]-lY[i]) + (pos[2]-lZ[i])*(pos[2]-lZ[i]));
    		potterm = -sqrt(tmp);
    		tmp *= (potterm) * lM[i];
    		potterm *= lM[i];
    		acc[0] += tmp*pos[0];
