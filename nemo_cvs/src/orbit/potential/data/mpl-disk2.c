...
{
    double apar, qpar, spar, ppar, lpar, rpar, rcyl, x, y, z;
    double i1,i2,i3;
...
// I did these derivatives in Mathematica, and will try to keep it consistent with the conventions written above
	...
	sprintf(commandx, "< integralx %f %f %f %f %f", x, y, z, disk_radius, h);
	sprintf(commandy, "< integraly %f %f %f %f %f", x, y, z, disk_radius, h);
	sprintf(commandz, "< integralz %f %f %f %f %f", x, y, z, disk_radius, h);

	double resultx, resulty, resultz;
	
        resultx = system(commandx);

        resulty = system(commandy);

        resultz = system(commandz);
