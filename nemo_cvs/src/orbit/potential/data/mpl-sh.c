...
local double q = 1.0;
local double d = 1.0;
local double halomass = 1.0;
local double halod = 1.0;
...
void inipotential (int *npar, double *par, string name)
{
    ...
    if (n>7) q = par[7];
    if (n>8) d = par[8];
    if (n>9) halomass = par[9];
    if (n>10) halod = par[10];
...
{
    double apar, qpar, spar, ppar, lpar, rpar, rcyl, ppar2, rpar2;
    int i;
...
// 20070507 bwillett used hypot from math.h
    ...
    rpar = ppar - plu_rc;
    ppar2 = sqrt ((pos[X]*pos[X])+(pos[Y]*pos[Y])+(pos[Z]*pos[Z])) + halod;
    rpar2 = ppar - halod;
    lpar = (rcyl*rcyl) + ((pos[Z]/q)*(pos[Z]/q)) + (d*d);
	
// 20070312 bwillett added plummer sphere potential and logarithmic potential
    *pot = (-(miya_mass)/spar) + (-(plu_mass)/ppar) + (-(halomass)/ppar2) + (vhalo*vhalo*log(lpar));
...
// I did these derivatives in Mathematica, and will try to keep it consistent with the conventions written above
	...
	acc[X] = - ( ( (2.0*vhalo*vhalo*pos[X])/(lpar) ) + ( (plu_mass*pos[X])/(rpar*ppar*ppar) ) + ( (halomass*pos[X])/(rpar2*ppar2*ppar2) ) + ( (miya_mass*pos[X])/(pow(spar,1.5)) ) );
	acc[Y] = - ( ( (2.0*vhalo*vhalo*pos[Y])/(lpar) ) + ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) + ( (halomass*pos[Y])/(rpar2*ppar2*ppar2) ) + ( (miya_mass*pos[Y])/(pow(spar,1.5)) ) );
	acc[Z] = - ( ( (2.0*vhalo*vhalo*pos[Z])/(lpar) ) + ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) + ( (halomass*pos[Z])/(rpar2*ppar2*ppar2) ) + ( (miya_mass*pos[Z]*apar) / (qpar*pow(spar,1.5)) ) );
