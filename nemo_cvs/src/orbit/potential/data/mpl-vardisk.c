...
void inipotential (int *npar, double *par, string name)
{
    ...
    if (n>7) q = par[7];
    if (n>8) d = par[8];
    if (n>9) diskend = par[9];
    if (n>10) warning("mpl: only first 10 parameters recognized");
...
// willeb 09112008 - this implements a hard disk cutoff of 15 kpc
// we're testing the extent of the disk
