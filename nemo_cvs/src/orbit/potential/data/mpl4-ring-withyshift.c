...
double mplaccx, mplaccy, mplaccz;
	...
	mplaccz = - ( ( (2.0*vhalo*vhalo*pos[Z])/(q*q*lpar) ) + ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Z]*apar) / (qpar*pow(spar,1.5)) ) );
        ...
        costheta = pos[Z] / sqrt(pos[X]*pos[X] + (pos[Y]-yshift)*(pos[Y]-yshift) + pos[Z]*pos[Z]);
        ...
        psi = atan2(pos[Y]-yshift,pos[X]);
        xi = sqrt(pos[X]*pos[X] + (pos[Y]-yshift)*(pos[Y]-yshift) + pos[Z]*pos[Z])/a;
    ...
    *pot = *pot + ((-G*ringmass)/(2*PI)) * fenergy(pos[X],pos[Y]-yshift,pos[Z],a);
...
double f1(double mu) {
	...
	for(i=0.0;i<=n;i+=1) {
		val = 0.0 + i*((PI - 0.0)/n);
		quant = 1.0 - mu*cos(val);
		if(i == 0.0 || i == n) {
			total += pow(quant, -1.5);
		} else {
			total += 2*pow(quant, -1.5);
		}
	}

	ans = ((PI - 0.0)/(2*n)) * total;
	return ans;
}
...
double f2(double mu) {
	...
	for(i=0.0;i<=n;i+=1) {
		...
		quant = 1.0 - mu*cos(val);
		if(i == 0.0 || i == n) {
			total += cos(val)*pow(quant, -1.5);
		} else {
			total += 2*cos(val)*pow(quant, -1.5);
		}
	}
