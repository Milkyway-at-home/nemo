...
arg = C1*pos[X]*pos[X] + C2*pos[Y]*pos[Y] + C3*pos[X]*pos[Y] + ((pos[Z]*pos[Z])/(qz*qz)) + d*d;
    ...
    *pot = (-(miya_mass)/sqrt(spar)) + (-(plu_mass)/ppar) + (vhalo*vhalo*log(arg));
	...
	acc[X] = - ( ( (vhalo*vhalo*(2.0*C1*pos[X] + C3*pos[Y]))/(arg) ) + ( (plu_mass*pos[X])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[X])/(pow(spar,1.5)) ) );
	acc[Y] = - ( ( (vhalo*vhalo*(2.0*C2*pos[Y] + C3*pos[X]))/(arg) ) + ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Y])/(pow(spar,1.5)) ) );
	acc[Z] = - ( ( (2.0*vhalo*vhalo*pos[Z])/(qz*qz*arg) ) + ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Z]*apar) / (qpar*pow(spar,1.5)) ) );
