...
// 20070312 bwillett added plummer sphere potential and logarithmic potential
    *pot = (-(miya_mass)/sqrt(spar)) + (-(plu_mass)/ppar) + (-(((halo_a*vcmax*vcmax)/(0.216))*log(1+(rpar/halo_a)))/rpar);
//    *pot = 1;
...
// 20090720 bwillett removed logarithmic dark matter halo and replaced with NFW spherical halo
	...
	acc[X] = - ( ( (plu_mass*pos[X])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[X])/(pow(spar,1.5)) ) ) + ((halo_a*vcmax*vcmax)/(0.216))*pos[X]*hpar;
	acc[Y] = - ( ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Y])/(pow(spar,1.5)) ) ) + ((halo_a*vcmax*vcmax)/(0.216))*pos[Y]*hpar;
	acc[Z] = - ( ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Z]*apar) / (qpar*pow(spar,1.5)) ) ) + ((halo_a*vcmax*vcmax)/(0.216))*pos[Z]*hpar;
}
