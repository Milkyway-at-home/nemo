...
void inipotential (int *npar, double *par, string name)
{
    ...
    if (n>5) vcmax = par[5];
    if (n>6) halo_a = par[6];
    if (n>7) warning("mpl: only first 7 parameters recognized");
...
// 20070312 bwillett added plummer sphere potential and logarithmic potential
    *pot = (-(disk_mass*(1-exp(-rpar/disk_b)))/rpar) + (-(plu_mass)/ppar) + (-(((halo_a*vcmax*vcmax)/(0.216))*log(1+(rpar/halo_a)))/rpar);
...
// 20090804 bwillett removed miyamoto disk and replaced with exponential disk 
	...
	acc[X] = - ( ( (plu_mass*pos[X])/(rpar*ppar*ppar) ) - pos[X]*diskpar ) + ((halo_a*vcmax*vcmax)/(0.216))*pos[X]*hpar;
	acc[Y] = - ( ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) - pos[Y]*diskpar ) + ((halo_a*vcmax*vcmax)/(0.216))*pos[Y]*hpar;
	acc[Z] = - ( ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) - pos[Z]*diskpar ) + ((halo_a*vcmax*vcmax)/(0.216))*pos[Z]*hpar;
}
