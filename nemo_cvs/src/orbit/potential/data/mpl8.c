...
{
    double apar, qpar, spar, ppar, lpar, rpar, rcyl, rsq;
    int i;
...
// I did these derivatives in Mathematica, and will try to keep it consistent with the conventions written above
	...
	acc[X] = - (( (plu_mass*pos[X])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[X])/(pow(spar,1.5)) ) ) + ((haloa*pos[X]*pow(rpar,-alpha))/(alpha-3.0));
	acc[Y] = - ( ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Y])/(pow(spar,1.5)) ) ) + ((haloa*pos[Y]*pow(rpar,-alpha))/(alpha-3.0));
	acc[Z] = - ( ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) + ( (miya_mass*pos[Z]*apar) / (qpar*pow(spar,1.5)) ) ) + ((haloa*pos[Z]*pow(rpar,-alpha))/(alpha-3.0));
