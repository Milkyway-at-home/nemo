...
void inipotential (int *npar, double *par, string name)
{
    ...
    if (n>9) phi = par[9];
    if (n>10) d = par[10];
    if (n>11) vhalo2 = par[11];
    if (n>12) qhalo2 = par[12];
    if (n>13) halod2 = par[13];
    if (n>14) warning("mpl: only first 14 parameters recognized");

}

void potential_double(int *ndim,double *pos,double *acc,double *pot,double *time)
{
    double apar, qpar, spar, ppar, rpar, rcyl,lpar, diskpar;
    double arg;
    int i;
...
arg = C1*pos[X]*pos[X] + C2*pos[Y]*pos[Y] + C3*pos[X]*pos[Y] + ((pos[Z]*pos[Z])/(qz*qz)) + d*d;
    ...
    *pot = (-(disk_mass*(1-exp(-rpar/disk_b)))/rpar) + (-(plu_mass)/ppar) + (vhalo*vhalo*log(arg)) + (vhalo2*vhalo2*log(lpar));

diskpar = (disk_mass * exp(-rpar/disk_b) * disk_b * (1.0 - exp(rpar/disk_b) + rpar/disk_b)) / (disk_b * rpar*rpar*rpar);

	acc[X] = - ( ( (vhalo*vhalo*(2.0*C1*pos[X] + C3*pos[Y]))/(arg) ) + ( (2.0*vhalo2*vhalo2*pos[X])/(lpar) ) + ( (plu_mass*pos[X])/(rpar*ppar*ppar) ) ) + pos[X]*diskpar;
	acc[Y] = - ( ( (vhalo*vhalo*(2.0*C2*pos[Y] + C3*pos[X]))/(arg) ) + ( (2.0*vhalo2*vhalo2*pos[Y])/(lpar) ) + ( (plu_mass*pos[Y])/(rpar*ppar*ppar) ) ) + pos[Y]*diskpar;
	acc[Z] = - ( ( (2.0*vhalo*vhalo*pos[Z])/(qz*qz*arg) ) + ( (2.0*vhalo2*vhalo2*pos[Z])/(qhalo2*qhalo2*lpar) ) + ( (plu_mass*pos[Z])/(rpar*ppar*ppar) ) ) + pos[Z]*diskpar;
