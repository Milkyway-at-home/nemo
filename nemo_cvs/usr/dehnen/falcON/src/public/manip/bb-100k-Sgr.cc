...
namespace falcON { namespace Manipulate {
  ...
  public:
	...
	double positions[100000][3];
	double SgrXY[100000][2];
	double velocities[100000][3];
	double simtime;
	...
	//OUT << "Hi from inside the bb manipulator to the file!\n";
  	LoopSubsetBodies(S,b) {
		...
		// Record the Sgr X-Y coordinates also
		SgrXY[i][0] = 0.998*positions[i][0] + 0.066*positions[i][1];
		SgrXY[i][1] = -(-0.015*positions[i][0] + 0.232*positions[i][1] - 0.972*positions[i][2]); // The extra negative sign is because Heidi's paper has the plot upside-down
	...
	for(mag=0;mag<=1;mag+=0.00001) {
		...
		acc[Y] = - ( ( (2.0*vhalo*vhalo*mag*worbitz)/(q*q*lpar) ) + ( (plu_mass*mag*worbitz)/(rpar*ppar*ppar) ) + ( (miya_mass*mag*worbitz)/(pow(spar,1.5)) ) );
