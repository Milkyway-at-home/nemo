...
namespace falcON { namespace Manipulate {
  ...
  public:
	...
	vel = pow(velocities[i][0]*velocities[i][0] + velocities[i][1]*velocities[i][1] + velocities[i][2]*velocities[i][2], 0.5);

	rt = pow(mass*rs*rs / (vel*vel), 0.33);
	...
	for(i=0;i<=N-1;i++) {
		...
		if(r > 2*rt) {
			// If r > 2*rt, the particle is unbound	
    			OUT << S->time() << "	" << i <<  "	" << totenergy << "	" << positions[i][0] << "	" << positions[i][1] << "	" << positions[i][2] << "	" << velocities[i][0] << "	" << velocities[i][1] << "	" << velocities[i][2] << "\n";
		}
	}
