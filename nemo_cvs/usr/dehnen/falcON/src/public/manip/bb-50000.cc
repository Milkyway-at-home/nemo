...
namespace falcON { namespace Manipulate {
  ...
  public:
	...
	double positions[50000][3];
	double velocities[50000][3];
	double simtime;
	...
	for(i=0;i<=N-1;i++) {
		...
		double totenergy, KE, PE, costheta;	
		...
		//  The angle theta is the projection of the particle's GC r vector into the z axis
		// We report cosine theta

		costheta = positions[i][2] / rpar;
	
		if(r > 2*2*rt) {
			// If r > 2*rt, the particle is unbound	
    			OUT << S->time() << "	" << i <<  "	" << totenergy << "	" << costheta << "\n";
		}
	}
