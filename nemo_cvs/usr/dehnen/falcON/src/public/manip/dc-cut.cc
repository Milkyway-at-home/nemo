...
namespace falcON { namespace Manipulate {
  ...
  public:
	...
	std::cerr<<" Manipulator \"dens_centre\" centre:\n"
		 ...
		 <<" par[1]: desired X location of core\n"
		 <<" par[2]: desired Y location of core\n"
		 <<" par[3]: desired Z location of core\n"
		 <<" par[4]: distance precision of core to (X,Y,Z)\n"
...
// willeb 11192008 I added this


	float dist;
	dist = pow((XCEN[0]-XW)*(XCEN[0]-XW) + (XCEN[1]-YW)*(XCEN[1]-YW) + (XCEN[2]-ZW)*(XCEN[2]-ZW),0.5);

	falcON_WarningN("%f %f %f dist=%f\n",XCEN[0],XCEN[1],XCEN[2], dist);
	if(dist < thresh) {
		falcON_THROW("UH OH, I'M KILLING THE PROGRAM BECAUSE WE'RE CLOSE ENOUGH TO THE ORBIT POINT\n");
	}
